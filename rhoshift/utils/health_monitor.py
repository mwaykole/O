...
class OperatorHealthMonitor:
    ...
    def check_operator_health(
        self, operator_name: str, namespace: str, include_dependencies: bool = True
        ...
        if include_dependencies:
            dep_status, dep_results = self._check_operator_dependencies(
                    ...
                    if condition.get("type") == "CatalogSourcesUnhealthy":
                        if condition.get("status") == "True":
                            unhealthy_subs.append(
                                f"{sub_name}: {condition.get('message', 'Catalog source unhealthy')}"
                            )

                    if (
                        "Error" in condition.get("type", "")
                        and condition.get("status") == "True"
                    ):
                        degraded_subs.append(
                            f"{sub_name}: {condition.get('message', 'Error condition')}"
                        )
    ...
    def _check_operatorgroup_health(self, check: HealthCheck) -> ResourceHealth:
        ...
        try:
            ...
            if len(operator_groups) > 1:
                og_names = [
                    og.get("metadata", {}).get("name", "unknown")
        ...
        try:
            ...
            for dsci in dscis:
                ...
                if phase == "Ready":
                    ...
                    error_conditions = [
                        c for c in conditions if c.get("status") == "False"
                    ]

                    if error_conditions:
                        error_msg = error_conditions[0].get(
    ...
    def _check_operator_dependencies(
...
def check_operator_health(
    operator_name: str, namespace: str, oc_binary: str = "oc"
) -> Tuple[HealthStatus, List[ResourceHealth]]:
