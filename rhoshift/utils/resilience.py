...
class ResilientOperatorManager:
    ...
    def execute_with_resilience(
        ...
        **kwargs,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(
                    f"Executing {operation_name} (attempt {attempt}/{max_retries})"
                )
                ...
                error_msg = str(e)
                logger.warning(
                    f"⚠️  {operation_name} failed (attempt {attempt}/{max_retries}): {error_msg}"
                )
                ...
                if error_pattern and attempt < max_retries:
                    ...
                    if recovery_result:
                        ...
                        if recovery_result.get("should_retry", True):
                            delay = (
                                error_pattern.backoff_multiplier ** (attempt - 1) * 5
                            )
                            logger.info(
                                f"🔄 Attempting recovery, retrying in {delay}s..."
                            )
                            time.sleep(delay)
                            continue
                    else:
    ...
    def _wait_for_certificates(context: Dict[str, Any], attempt: int) -> Dict[str, Any]:
        ...
        return {
            "should_retry": True,
            "warnings": [
                f"Webhook certificate timing issue resolved after {delay}s wait"
            ],
        }

    @staticmethod
    def _handle_resource_conflict(
        error_msg: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "already exists" in error_msg.lower():
            logger.info(
                "🔍 Resource already exists, checking if it matches desired state..."
            )
    ...
    def _resolve_operator_group_conflict(context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"🔧 Attempting to resolve OperatorGroup conflicts in {namespace}..."
        )
            ...
            try:
                ...
                if len(og_data.get("items", [])) > 1:
                    ...
                    return {
                        "should_retry": False,
                        "warnings": [
                            f"Multiple OperatorGroups in {namespace} - manual intervention required"
                        ],
                    }
    ...
    def _check_storage_requirements(context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if rc == 0:
            ...
            if available_pvs == 0:
                warnings.append(
                    "No available PersistentVolumes - may cause deployment issues"
                )
    ...
    def _validate_image_access(
        ...
        if match:
            ...
            warnings.append(
                f"Image pull failed for {image_name} - check registry access and credentials"
            )
        else:
...
class PreflightChecker:
    ...
    def validate_permissions(
        oc_binary: str = "oc", namespace: str = "default"
    ) -> Tuple[bool, List[str]]:
        ...
        for resource in required_resources:
            for verb in required_verbs:
                ...
                if rc != 0 or "no" in stdout.lower():
                    warnings.append(
                        f"Missing {verb} permission for {resource} in {namespace}"
                    )
                ...
                for quota in quotas.get("items", []):
                    ...
                    for resource, limit in hard.items():
                        ...
                        if resource in ["pods", "requests.cpu", "requests.memory"]:
                            warnings.append(
                                f"Resource quota in {namespace}: {resource} = {current_usage}/{limit}"
                            )
            except json.JSONDecodeError:
...
def run_preflight_checks(oc_binary: str = "oc") -> Tuple[bool, List[str]]:
    ...
    success, warnings = PreflightChecker.validate_permissions(
        oc_binary, "openshift-operators"
    )
    all_warnings.extend(warnings)
    if not success:
        return False, all_warnings
    ...
    # Resource quotas
    success, warnings = PreflightChecker.check_resource_quotas(
        oc_binary, "openshift-operators"
    )
    all_warnings.extend(warnings)
