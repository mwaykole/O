...
from rhoshift.utils.constants import OpenShiftOperatorInstallManifest, WaitTime
...
class OpenShiftOperatorInstaller:
    ...
    def get_operator_configs(cls, oc_binary: str = "oc"):
        ...
        for op_key in OpenShiftOperatorInstallManifest.list_operators():
            ...
            configs[op_key] = {
                "manifest": manifest_generator.generate_operator_manifest(
                    op_key, oc_binary
                ),
    ...
    def install_operator(cls, operator_name: str, **kwargs) -> Tuple[int, str, str]:
        ...
        if operator_name not in configs:
            ...
            raise ValueError(
                f"Unknown operator: {operator_name}. Available: {available}"
            )
        ...
        if operator_name == "openshift-cert-manager-operator":
            ...
            )  # 10 minutes for cert-manager
            logger.info(
                f"Using extended timeout for cert-manager operator: {kwargs['timeout']} seconds"
            )
    ...
    def install_keda_operator(cls, **kwargs) -> Tuple[int, str, str]:
        ...
        result = cls.install_operator(
            "openshift-custom-metrics-autoscaler-operator", **kwargs
        )

        if result[0] == 0:
            logger.info(
                "Operator installed successfully. Creating KedaController resource..."
            )
...
"""
            ...
            try:
                ...
                if rc == 0:
                    ...
                    while time.time() < end_time:
                        try:
                            rc, phase, stderr = run_command(
                                keda_ready_cmd, log_output=False
                            )
                        ...
                        logger.warning(
                            "⚠️  KEDA may still be starting up. Check status with: oc get kedacontroller keda -n openshift-keda"
                        )

                else:
                    logger.error(
                        f"❌ Failed to create KedaController resource: {stderr}"
                    )
                    logger.warning(
                        "You can manually create the KedaController later if needed"
                    )
    ...
    def install_rhoai_operator(
        cls, oc_binary: str = "oc", timeout: int = 1200, **kwargs
        ...
        try:
            ...
            extra_params = (
                " -n rhods-operator -p opendatahub-operators"
            ...
            if create_dsc_dsci:
                logger.info(
                    "⏳ Waiting for RHOAI webhook certificates to become valid..."
                )
                webhook_wait_time = (
                    60  # Wait 60 seconds for certificates to become valid
                )
                logger.info(
                    f"⏱️  Waiting {webhook_wait_time} seconds for webhook certificates..."
                )
            ...
            results["rhods-operator"] = {"status": "failed", "message": str(e)}
            return results
    ...
    def _install_operator(
        cls, operator_name: str, manifest: str, **kwargs
    ) -> Tuple[int, str, str]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Installation failed with exit code {rc}. Error: {stderr}"
            )
    ...
    def _check_operator_status(
        ...
        try:
            if namespace not in [
                "openshift-operators"
            ]:  # Skip conflict check for shared namespaces
                ...
                if rc == 0:
                    ...
                    if len(lines) > 2:  # More than header + 1 operator group
                        return (
                            False,
                            f"Multiple OperatorGroups detected in namespace {namespace}. This prevents CSV creation.",
                        )
            ...
            if rc == 0:
                ...
                for sub in subs.get("items", []):
                    ...
                    for condition in conditions:
                        if condition.get(
                            "status"
                        ) == "True" and "Error" in condition.get("type", ""):
                            return (
                                False,
                                f"Subscription error: {condition.get('message', 'Unknown error')}",
                            )
            ...
            try:
                operator_config = OpenShiftOperatorInstallManifest.get_operator_config(
                    operator_name
                )
            except ValueError:
                pass
            ...
            try:
                ...
                if not matching_csvs:
                    return (
                        False,
                        f"No CSV found matching pattern '{csv_search_pattern}' in namespace {namespace}",
                    )

                succeeded_csvs = [
                    csv for csv, phase in matching_csvs if phase == "Succeeded"
                ]
                if succeeded_csvs:
                    logger.debug(f"Found succeeded CSV: {succeeded_csvs[0]}")
                    return (
                        True,
                        f"Operator fully installed and ready (CSV: {succeeded_csvs[0]})",
                    )

                # Report status of non-succeeded CSVs
                failed_csvs = [
                    (csv, phase)
                    for csv, phase in matching_csvs
                    if phase in ["Failed", "InstallReady"]
                ]
                if failed_csvs:
                    return (
                        False,
                        f"CSV in failed state: {failed_csvs[0][0]} - {failed_csvs[0][1]}",
                    )
                ...
                if pending_csvs:
                    return (
                        False,
                        f"CSV pending: {pending_csvs[0][0]} - {pending_csvs[0][1]}",
                    )
        ...
        def _check_operator(op: Dict[str, str]) -> Tuple[str, bool, str]:
            ...
            while time.time() < end_time:
                is_ready, message = cls._check_operator_status(
                    op["name"], op["namespace"], oc_binary, end_time, interval
                )
            ...
            return (
                op["name"],
                False,
                f"Timeout after {timeout} seconds waiting for {op['name']}",
            )

        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_op = {
                executor.submit(_check_operator, op): op["name"] for op in operators
            }
    ...
    @classmethod
    def force_delete_rhoai_dsc_dsci(
        cls,
        oc_binary: str = "oc",
        ...
        **kwargs,
        ...
        results = {}
        ods_namespace = (
            "opendatahub" if channel == "odh-nightlies" else "redhat-ods-operator"
        )
        application_namespace = (
            "opendatahub" if channel == "odh-nightlies" else "redhat-ods-applications"
        )
        ...
        for cmd_info in commands:
            try:
                rc, stdout, stderr = run_command(
                    cmd_info["cmd"], timeout=timeout, **kwargs
                )
                ...
                if rc != 0:
                    logger.warning(
                        f"Command failed: {cmd_info['name']}. Error: {stderr}"
                    )

                    # Special handling for webhook certificate issues in DSCI deletion
                    if cmd_info["name"] == "delete_dsci" and (
                        "webhook" in stderr.lower()
                        or "certificate" in stderr.lower()
                        or "x509" in stderr.lower()
                    ):
                        logger.warning(
                            "⚠️  DSCI deletion failed due to webhook certificate issues, trying alternative approach..."
                        )
                        try:
                            ...
                            alt_rc, alt_stdout, alt_stderr = run_command(
                                direct_delete_cmd, timeout=60
                            )
                            if alt_rc == 0:
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"⚠️  Finalizer patch approach also failed: {patch_error}"
                                    )
                        except Exception as e:
    ...
    def deploy_dsc_dsci(
        ...
        if create_dsc_dsci:
            ...
            for cmd_name, cmd_result in result.items():
                ...
                if cmd_result["status"] != "success":
                    logger.error(
                        f" {cmd_result.get('stderr', cmd_result.get('error', ''))}"
                    )
                    # If DSCI deletion fails due to webhook issues, try alternative deletion
                    if (
                        cmd_name == "delete_dsci"
                        and "webhook" in cmd_result.get("stderr", "").lower()
                    ):
                        logger.warning(
                            "⚠️  DSCI deletion failed due to webhook issues, trying alternative approach..."
                        )
                        try:
                            # Try direct resource deletion bypassing webhook validation
                            direct_delete_cmd = "oc delete dsci default-dsci --ignore-not-found=true --force --grace-period=0"
                            rc, stdout, stderr = run_command(
                                direct_delete_cmd, timeout=60
                            )
                            if rc == 0:
                                ...
                                logger.warning(
                                    f"⚠️  Direct DSCI deletion also failed: {stderr}"
                                )
                        except Exception as e:
                            logger.warning(f"⚠️  Alternative DSCI deletion failed: {e}")
            ...
            if dsci_deletion_successful or create_dsc_dsci:
                ...
                while time.time() - start_time < max_wait_time:
                    check_cmd = (
                        "oc get dsci default-dsci 2>/dev/null || echo 'NOT_FOUND'"
                    )
                    rc, output, _ = run_command(check_cmd, log_output=False)

                    if rc != 0 or "NOT_FOUND" in output:
                        logger.info(
                            "✅ DSCI deletion confirmed - resource no longer exists"
                        )
                        break
                    else:
                        logger.debug(
                            f"DSCI still exists, waiting... ({int(time.time() - start_time)}s elapsed)"
                        )
                ...
                if rc == 0 and "NOT_FOUND" not in output:
                    logger.warning(
                        "⚠️  DSCI still exists after deletion attempts - proceeding anyway"
                    )
        ...
        if rc == 0 and existing_monitoring_ns.strip() != "NOT_FOUND":
            ...
            if existing_monitoring_ns != desired_monitoring_ns:
                logger.warning(
                    f"⚠️  DSCI conflict detected: existing monitoring namespace is '{existing_monitoring_ns}', but '{desired_monitoring_ns}' is required for channel '{channel}'"
                )

                if create_dsc_dsci:
                    logger.info(
                        "🔄 Attempting to recreate DSCI with correct configuration..."
                    )
                    # Force delete was attempted above during create_dsc_dsci block
                    logger.info(
                        "🛠️  Will attempt to apply new DSCI - if deletion failed, this may result in immutable field error"
                    )
                else:
                    logger.info(
                        f"ℹ️  Using existing DSCI with monitoring namespace '{existing_monitoring_ns}' (channel '{channel}' requested '{desired_monitoring_ns}')"
                    )
        ...
        if should_apply_dsci:
            dsci = constants.get_dsci_manifest(kserve_raw=kserve_raw, **dsci_params)
            ...
            try:
                ...
                error_msg = str(e)
                if (
                    "MonitoringNamespace is immutable" in error_msg
                    or "immutable" in error_msg
                ):
                    logger.error(f"❌ DSCI immutable field conflict: {error_msg}")
                    logger.info(
                        "💡 Suggestion: Use --deploy-rhoai-resources to force recreate DSCI with correct configuration"
                    )
                    raise
                else:
                    logger.error(
                        f"❌ DSCI creation failed with unexpected error: {error_msg}"
                    )
                    raise
        else:
            logger.info(
                f"ℹ️  Using existing DSCI (monitoring namespace: {existing_monitoring_ns})"
            )
    ...
    def uninstall_operator(
        cls, operator_name: str, namespace: str, oc_binary: str = "oc", **kwargs
    ) -> Tuple[int, str, str]:
        try:
    ...
    @classmethod
    def install_all_and_wait(
        cls, oc_binary="oc", **kwargs
    ) -> Dict[str, Dict[str, str]]:
        """
        ...
        for op_key in available_operators:
            ...
            if hasattr(cls, method_name):
                install_methods.append(
                    (op_key, op_config.namespace, getattr(cls, method_name))
                )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(install_methods)
        ) as executor:
            futures = [
