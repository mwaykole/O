...
class StabilityCoordinator:
    ...
    def install_operator_with_stability(
        self, operator_name: str, namespace: str, installation_func: Callable, **kwargs
        ...
        try:
            ...
            if self.config.enable_preflight_checks:
                ...
                preflight_success, preflight_warnings = self._run_preflight_checks(
                    namespace
                )
            ...
            if self.config.level.value >= StabilityLevel.ENHANCED.value:
                install_success, install_result, install_warnings = (
                    execute_resilient_operation(
                        installation_func,
            ...
            if self.config.enable_health_monitoring:
                ...
                health_success, health_details = self._run_health_checks(
                    operator_name, namespace
                )
                results["health_check_passed"] = health_success
                results["health_status"] = health_details.get(
                ...
                if not health_success and self.config.enable_auto_recovery:
                    ...
                    recovery_success = self._attempt_auto_recovery(
                        operator_name, namespace, health_details
                    )
    ...
    def _run_preflight_checks(self, namespace: str) -> Tuple[bool, List[str]]:
        ...
        success, warnings = self.preflight_checker.validate_cluster_connectivity(
            self.oc_binary
        )
        ...
        success, warnings = self.preflight_checker.validate_permissions(
            self.oc_binary, namespace
        )
        all_warnings.extend(warnings)
        if not success:
            return False, all_warnings
        ...
        success, warnings = self.preflight_checker.check_resource_quotas(
            self.oc_binary, namespace
        )
        all_warnings.extend(warnings)
    ...
    def _check_namespace_readiness(self, namespace: str) -> List[str]:
        ...
        if rc != 0:
            if "not found" in stderr.lower():
                logger.info(
                    f"📁 Namespace {namespace} will be created during installation"
                )
            else:
    ...
    def _run_health_checks(
        self, operator_name: str, namespace: str
    ) -> Tuple[bool, Dict[str, Any]]:
        ...
        health_details = {
            ...
            "healthy_count": len(
                [h for h in health_results if h.status == HealthStatus.HEALTHY]
            ),
            "degraded_count": len(
                [h for h in health_results if h.status == HealthStatus.DEGRADED]
            ),
            "unhealthy_count": len(
                [h for h in health_results if h.status == HealthStatus.UNHEALTHY]
            ),
            "unknown_count": len(
                [h for h in health_results if h.status == HealthStatus.UNKNOWN]
            ),
        }
        ...
        if success:
            logger.info(
                f"✅ Health checks passed for {operator_name} (status: {overall_status.value})"
            )
        else:
            logger.warning(
                f"⚠️  Health checks failed for {operator_name} (status: {overall_status.value})"
            )
    ...
    def _attempt_auto_recovery(
        self, operator_name: str, namespace: str, health_details: Dict[str, Any]
    ) -> bool:
        ...
        success_count = 0
        for action_type, health_result in recovery_actions[
            : self.config.max_recovery_attempts
        ]:
            logger.info(
                f"🔧 Attempting recovery action: {action_type} for {health_result.name}"
            )
    ...
    def _execute_recovery_action(
        self, action_type: str, health_result: ResourceHealth, namespace: str
    ) -> bool:
    ...
    def _restart_subscription(
        self, health_result: ResourceHealth, namespace: str
    ) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Subscription restart not fully implemented - requires manifest recreation"
            )
            return False
        ...
        try:
            ...
            if not deployment_name:
                ...
                if health_result.resource_type == ResourceType.DEPLOYMENT:
                    deployment_name = health_result.name.replace(
                        "-deployment", ""
                    ).split("-")[-1]

            if deployment_name:
                logger.info(
                    f"🔄 Restarting deployment {deployment_name} in {namespace}..."
                )
                ...
                if rc == 0:
                    ...
                    logger.error(
                        f"Failed to restart deployment {deployment_name}: {stderr}"
                    )
    ...
    def monitor_installed_operators(self) -> Dict[str, Dict[str, Any]]:
        ...
        for operator_name, operator_info in self.installed_operators.items():
            ...
            try:
                ...
                self.installed_operators[operator_name]["health_status"] = (
                    health_details.get("overall_status", HealthStatus.UNKNOWN)
                )
    ...
    def generate_stability_report(self) -> str:
        ...
        if self.installed_operators:
            ...
            for op_name, op_info in self.installed_operators.items():
                ...
                time_str = (
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(install_time))
                    if install_time
                    else "Unknown"
                )

                report.append(
                    f"  {status_emoji} {op_name} ({op_info['namespace']}) - Installed: {time_str}"
                )

        report.append("")
        report.append("CONFIGURATION:")
        report.append(
            f"  • Pre-flight checks: {'Enabled' if self.config.enable_preflight_checks else 'Disabled'}"
        )
        report.append(
            f"  • Health monitoring: {'Enabled' if self.config.enable_health_monitoring else 'Disabled'}"
        )
        report.append(
            f"  • Auto recovery: {'Enabled' if self.config.enable_auto_recovery else 'Disabled'}"
        )
...
def create_stability_coordinator(
    level: StabilityLevel = StabilityLevel.ENHANCED, oc_binary: str = "oc"
