...
def parse_args() -> argparse.Namespace:
    ...
    operator_group.add_argument(
        "--rhoai", action="store_true", help="Install RHOAI Operator"
    )
    operator_group.add_argument(
        "--kueue",
        nargs="?",
    ...
    config.add_argument(
        "--oc-binary", default="oc", help="Path to oc CLI (default: oc)"
    )
    config.add_argument(
    ...
    config.add_argument(
        "--raw", default=False, type=str_to_bool, help="True if install raw else False"
    )
    config.add_argument(
        "--rhoai-image",
        required="--rhoai" in sys.argv,
        type=str,
