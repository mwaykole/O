...
class TestOpenShiftOperatorInstaller:
    ...
    def test_install_cert_manager_operator(self):
        ...
        with patch.object(
            ...
            result = (
                OpenShiftOperatorInstaller.install_openshift_cert_manager_operator()
            )
    ...
    @patch("rhoshift.utils.utils.wait_for_resource_for_specific_status")
    @patch("time.sleep")
    def test_deploy_dsc_dsci_success(
        self, mock_sleep, mock_wait, mock_apply, mock_run_command
    ):
        ...
        OpenShiftOperatorInstaller.deploy_dsc_dsci(
            channel="stable", kserve_raw=False, create_dsc_dsci=False
        )
    ...
    @patch("rhoshift.utils.utils.apply_manifest")
    @patch("time.sleep")
    def test_deploy_dsc_dsci_with_existing_conflict(
        self, mock_sleep, mock_apply, mock_run_command
    ):
    ...
    @patch("rhoshift.utils.utils.apply_manifest")
    @patch(
        "rhoshift.utils.operator.operator.OpenShiftOperatorInstaller.force_delete_rhoai_dsc_dsci"
    )
    def test_deploy_dsc_dsci_with_recreation(
        self, mock_force_delete, mock_apply, mock_run_command
    ):
    ...
    def test_deploy_dsc_dsci_webhook_certificate_retry(self, mock_sleep, mock_apply):
        ...
        with patch("rhoshift.utils.utils.run_command") as mock_run_command:
            ...
            with patch(
                "rhoshift.utils.utils.wait_for_resource_for_specific_status"
            ) as mock_wait:
                mock_wait.return_value = (True, "Ready", "")

                OpenShiftOperatorInstaller.deploy_dsc_dsci(
                    channel="stable", create_dsc_dsci=False
                )
    ...
    def test_deploy_dsc_dsci_with_kueue_management_state(self):
            ...
            with patch("rhoshift.utils.utils.apply_manifest") as mock_apply:
                with patch(
                    ...
                    with patch("time.sleep"):
                        OpenShiftOperatorInstaller.deploy_dsc_dsci(
                            channel="stable", kueue_management_state="Managed"
                        )
        ...
        dsc_calls = [
            ...
            if "DataScienceCluster" in str(call)
        ]
...
class TestPerformanceAndConcurrency:
    ...
    def test_memory_efficiency_with_large_manifests(self):
        ...
        with patch(
            "rhoshift.utils.constants.OpenShiftOperatorInstallManifest.generate_operator_manifest"
        ) as mock_gen:
...
class TestIntegrationScenarios:
    ...
    @patch("rhoshift.utils.utils.wait_for_resource_for_specific_status")
    @patch("time.sleep")
    def test_complete_rhoai_installation_workflow(
        self, mock_sleep, mock_wait, mock_apply
    ):
