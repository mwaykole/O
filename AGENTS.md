...
- Cleanup preserves platform-managed OpenShift Service Mesh 3 (`servicemeshoperator3`): `scripts/cleanup/cleanup_all.sh` skips SM3 CSVs cluster-wide, avoids deleting every InstallPlan in `openshift-operators` (only targets rhoshift-managed operator InstallPlans there), and avoids Istio/sail CRD teardown plus broad `istio` webhook matching so ingress-owned SM3 does not end up stuck in `Pending`.
